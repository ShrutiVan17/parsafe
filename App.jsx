...
import AskParSafe from '.AskParSafe'
...
export default function App() {
  ...
  const selected =
    items.find((item) => item.parking_id === selectedId) ||
    items[0] ||
    null

  async function loadInitial() {
    try {
      const [ranked, currentStats] = await Promise.all([
        rankParking(prefs),
        getStats(),
      ])
    ...
    <>
      ...
      <header>
        <div className="brand">
          <b>PS</b>
          <span>
            <strong>ParSafe Project</strong>
            <small>React + Python Parking Decision System</small>
          </span>
        </div>
      ...
      <main>
        ...
        <section className="section">
          <Controls
            ...
            staticMode={!apiEnabled}
          />
        </section>

        <section className="section" id="rank">
          <div className="section-title">
            <div className="eyebrow">EXPLAINABLE TOP 5</div>
          ...
          <Recommendations
            items={items}
            selected={selected}
            onSelect={(item) => setSelectedId(item.parking_id)}
          />
        </section>
        ...
        <section className="section" id="retrieval">
          <AskParSafe apiEnabled={apiEnabled} />
        </section>
      </main>
